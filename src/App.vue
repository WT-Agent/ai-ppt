  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">汇报场景</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">听众对象</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">PPT 金字塔逻辑与视觉排版指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 演示文稿PPT大纲与逻辑策划专家');
...
const metricsList = [
  {
    "key": "slideHierarchyLogic",
    "label": "页面层级逻辑性"
  },
  {
    "key": "visualLayoutGuidance",
    "label": "视觉排版指导性"
  },
  {
    "key": "coreValueHighlight",
    "label": "核心价值突出度"
  },
  {
    "key": "audienceEngagementScore",
    "label": "听众吸引力评分"
  },
  {
    "key": "presentationTimeFit",
    "label": "演讲时长契合度"
  }
];
const guideRules = [
  {
    "title": "一页一结论原则",
    "advice": "每页幻灯片标题必须是一句有观点的结论句，而非仅仅是“项目背景”等名词",
    "avoid": "切忌在页面中堆砌密密麻麻的纯文本"
  },
  {
    "title": "数据可视化优先",
    "advice": "尽量将对比数据转化为柱状图、饼图或流程图，辅以大字号数字强调关键成果",
    "avoid": "避免用纯表格罗列未经过精简的原始数据"
  },
  {
    "title": "金字塔结构贯穿",
    "advice": "遵循“结论先行-归类分组-逻辑递进”架构，确保听众在前 3 页就能听懂核心逻辑",
    "avoid": "严禁将核心结论隐藏在最后几页才透露"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[PPT_SCORES\][\s\S]*?\[\/PPT_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[PPT_SCORES\](.*?)\[\/PPT_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('ppt_history_records');
...
const saveHistory = () => {
  localStorage.setItem('ppt_history_records', JSON.stringify(historyList.value));
};
